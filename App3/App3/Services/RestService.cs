    ...
    public class RestService
    {
        ...
        public async Task<RootMsg> GetMensagensListAsync(string idrecetor)
        {
            try
            {
                ...
                var response = await clientFora.GetStringAsync(url);
                var produtos = JsonConvert.DeserializeObject<RootMsg>(response);
                var produtosO = JsonConvert.DeserializeObject<RootMsgO>(response);
        ...
        public async Task<RootMsg> GetMensagensListAsyncTest(string idrecetor)
        {
            try
            {
                clientFora.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", await SecureStorage.GetAsync("tokenuser"));
        ...
        public async Task<HttpResponseMessage> MarcarMensagemLidoAsync(Mensagem msg)
        {
            try
            {
                clientFora.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", await SecureStorage.GetAsync("tokenuser"));
                string url = "http://tze.ddns.net:8070/api/Mensagens/MarcarLido";
                Console.WriteLine(url);
                var x = JsonConvert.SerializeObject(msg);
